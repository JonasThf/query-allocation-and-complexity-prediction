...
def load_and_prepare_dataset(file_path):
    ...
    for i in range(1, 11):  # Responses start from r_t5_xl_1 to r_t5_xl_10
        df[f'r_t5_base_{i}'] = [""] * len(df)
        df[f'scores_t5_base_{i}'] = [{"bart": 0.0, "rouge1": 0.0, "rouge2": 0.0, "rougeL": 0.0,
    ...
    for i in tqdm(range(start_index, end_index), desc=f"Processing Queries with {model.name_or_path}"):
        try:
            ...
            for j, response in enumerate(responses, start=1):
                df.at[i, f'r_t5_base_{j}'] = response

            # Store the average response time
            df.at[i, 'rt_t5_base_avg'] = float(avg_response_time)
...
# Log GPU status
print("Device status:")
log_file = "output_t5_base_10k.log"
...
output_file_path = '../../datasets/generated/step_7/train_data_with_50kx10_t5_base_answers_with_avgtime_max5000.jsonl'
