...
def load_and_prepare_dataset(file_path):
    ...
    for i in range(1, 11):  # Responses start from r_t5_xl_1 to r_t5_xl_10
        df[f'r_t5_xl_{i}'] = [""] * len(df)
        df[f'scores_t5_xl_{i}'] = [{"bart": 0.0, "rouge1": 0.0, "rouge2": 0.0, "rougeL": 0.0,
...
# Updated function to process queries and store responses in the DataFrame
def process_queries_with_multiple_responses(model, tokenizer, df, start_index, end_index, device, num_responses, max_new_tokens):
    ...
    for i in tqdm(range(start_index, end_index), desc=f"Processing Queries with {model.name_or_path}"):
        try:
            # Construct the query using the construct_query function
            ...
            for j, response in enumerate(responses, start=1):
                df.at[i, f'r_t5_xl_{j}'] = response

            # Store the average response time
            df.at[i, 'rt_t5_xl_avg'] = float(avg_response_time)
...
model_path_t5_xl = "google/flan-t5-xl"
print("Loading Flan-T5-XL model...")
t5_model = AutoModelForSeq2SeqLM.from_pretrained(model_path_t5_xl).to(device)
t5_tokenizer = AutoTokenizer.from_pretrained(model_path_t5_xl)
t5_tokenizer.pad_token = t5_tokenizer.eos_token
print("Loading complete.")

# Log GPU status
print("Device status:")
log_file = "output_t5_xl_10k.log"
...
output_file_path = '../../datasets/generated/step_7/train_data_with_50k_times_10_t5_xl_answers_with_avgtime_max5000.jsonl'
