...
def load_and_prepare_dataset(split, file_path):
    ...
    df['scores_t5_base'] = [metrics_template.copy() for _ in range(len(df))]
    ...
    df['discrepancy_base_vs_large'] = [0] * len(df)
    df['discrepancy_large_vs_xl'] = [0] * len(df)
    df['discrepancy_xl_vs_base'] = [0] * len(df)
...
empty_t5_base_answers_df = train_df.copy()

print("Loading flan-t5-base model...")
model_path_t5_base = "google/flan-t5-base"
t5_model = AutoModelForSeq2SeqLM.from_pretrained(model_path_t5_base).to(device)
t5_tokenizer = AutoTokenizer.from_pretrained(model_path_t5_base)
t5_tokenizer.pad_token = t5_tokenizer.eos_token

t5_output_column = 'r_t5_base'
t5_time_column = 'rt_t5_base'
max_new_tokens = 5000
start_index = 0
end_index = 100

print("Running on ", device)
filled_t5_base_answers_df = process_single_queries_with_timing(t5_model, t5_tokenizer, empty_t5_base_answers_df, start_index, end_index, t5_output_column, t5_time_column, max_new_tokens, device)

# Output file path
output_file_path = 'mix-instruct/train_data_with_100_t5_base_answers_with_time_max5000.json'

# Write the updated DataFrame to the output file
print("Writing the final output to file...")
filled_t5_base_answers_df.to_json(output_file_path, orient='records', lines=True)
